...
$(()=>{
    var carListGoods=JSON.parse(localStorage.carListGoods)
    carListGoods.forEach((item,i)=>{
        // console.log(item)
        ele=`				<div class="item">
					<div class="check">
        ...
        $('.num a').eq(1).click(function(){
            ...
            LXH_num++
            $(this).prev().val(LXH_num)
            //更新数量同时修改小计
            $(this).parents('.item').find('.count span').html(($(this).parents('.item').find('.price span').html()*$(this).prev().val()).toFixed(2))
            // console.log(($(this).parents('.item').find('.price span').html()*$(this).prev().val()).toFixed(2))
        ...
        $('#check1').click(function(){check_all()});
        $('#check2').click(function(){check_jd()});
        //自营勾选逻辑
        function check_jd(){
        ...
        $('.item .check input').click(function isCheckAll(){
            ...
            console.log(typeof(toBoolean(it)))
            if(!toBoolean(it)){
        ...
        $('.settlement button').click(function(){
            ...
            $('.total .user').text(localStorage.user)
